...
def _print_table(results: list[Result], *, runs: int) -> None:
    print(f"\nMedian of {runs} runs after warm-up (SQLite, Django test client):\n")
...
def run_benchmark(*, sessions: int, runs: int, reseed: bool) -> list[Result]:
    ...
    _print_table(results, runs=runs)
    return results
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_benchmark(sessions=args.sessions, runs=args.runs, reseed=args.reseed)
    return 0
