...
The logic below is upstream's, re-synced to djlint 1.45.2. What changed: the
regex marked EDITED, which is the point of the copy; keyword-only signatures,
which this repo requires past two parameters; and `run`'s closer branch
lifted into `_close` and two helpers, because upstream's single function is
over the complexity limit here. The predicates, their order and the shape of
the walk are untouched, so a re-sync stays a readable diff.

1.45.2 added `opened_names`, tracking every tag name opened by any tag —
including ones this rule doesn't model as an indent/unindent pair — so a
closer for a name djlint never modeled as an opener isn't reported as an
orphan. That's a different fix than this file's own (issue #824, still
open): it stops false orphans for *unmodeled* tags, not for the `end_`
spelling this copy exists to accept.
...
_BLOCK_LABEL_PATTERN: Final = re.compile(
    r"\{%[-+]?\s*(?:end)?block\s+([^\s%+-](?:[^\s%]*[^\s%+-])?)", cache_pattern=False
)
...
def _close(
    *,
    ...
    # a tag djlint never models as an opener still opens `name` generically;
    # reporting this closer as orphaned would call every custom block one
...
def run(
    ...
    **kwargs: Any,
    ...
    for match in _TEMPLATE_TAG_PATTERN.finditer(html):
        ...
        # a tag this rule doesn't track as an indent/unindent pair still
        # counts as having "opened" its name, for the orphan check above
        elif (open_name := _OPEN_NAME_PATTERN.match(tag)) is not None:
            opened_names.add(open_name.group(1))
