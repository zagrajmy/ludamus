...
class TestMakeUniqueSlug:
    def test_returns_base_slug_when_available(self) -> None:
        slug = make_unique_slug(
            name="Hello World", default="session", check_exists=lambda _s: False
        )
    ...
    def test_falls_back_to_default_for_empty_slug(self) -> None:
        slug = make_unique_slug(
            name="!!!", default="session", check_exists=lambda _s: False
        )
    ...
    def test_caps_long_name_to_column_length(self) -> None:
        ...
        slug = make_unique_slug(
            name="a" * 60, default="session", check_exists=lambda _s: False
        )

        assert len(slug) <= _SLUG_MAX_LENGTH

    def test_caps_long_default_fallback(self) -> None:
        # An empty name falls back to the default, which must also be capped so
        # an over-long default can't overflow the column.
        slug = make_unique_slug(
            name="", default="d" * 60, check_exists=lambda _s: False
        )
    ...
    def test_disambiguated_slug_still_fits_column(self) -> None:
        ...
        slug = make_unique_slug(name="a" * 60, default="session", check_exists=exists)
