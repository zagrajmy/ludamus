"""Integration tests for the Google Forms proposal importer.

`google.auth` is mocked at the package boundary (credentials + authorized
session); the importer's own `check` / `probe` logic runs for real, so the
...
from ludamus.links.google_forms import (
    FORMS_API_URL,
