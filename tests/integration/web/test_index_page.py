...
class TestEventsPageView:
    ...
    def event_fixture(self, non_root_sphere):
    ...
    @pytest.fixture(name="panel_access_user", params=["manager", "superuser"])
    def panel_access_user_fixture(self, request, active_user, non_root_sphere):
        # Panel access has to be on the sphere being served, not the root one.
    ...
    def test_session_count_counts_scheduled_sessions(self, client, non_root_sphere):
    ...
    def test_session_count_matches_public_schedule(self, client, non_root_sphere):
        event = EventFactory(sphere=non_root_sphere)
        space = SpaceFactory(event=event)
    ...
    def test_ok_with_same_day_event(self, client, non_root_sphere, faker):
        ...
        event = EventFactory(sphere=non_root_sphere, start_time=start, end_time=end)
    ...
    def test_ok_with_multi_month_event(self, client, non_root_sphere, faker):
    ...
    def test_ok_with_multi_year_event(self, client, non_root_sphere):
    ...
    def test_upcoming_events_sorted_soonest_first(self, client, non_root_sphere):
        now = datetime.now(UTC)
        far = EventFactory(
            sphere=non_root_sphere,
    ...
    def test_past_events_sorted_most_recent_first(self, client, non_root_sphere):
        now = datetime.now(UTC)
        older = EventFactory(
            sphere=non_root_sphere,
            start_time=now - timedelta(days=30),
    ...
    def test_mixed_buckets_each_sorted_independently(self, client, non_root_sphere):
        now = datetime.now(UTC)
        far = EventFactory(
            sphere=non_root_sphere,
            start_time=now + timedelta(days=30),
    ...
    def test_published_announcement_shown(self, client, non_root_sphere):
        announcement = Announcement.objects.create(
            sphere=non_root_sphere, title="Welcome", content="Hello there"
        )
    ...
    def test_draft_announcement_hidden(self, client, non_root_sphere):
        Announcement.objects.create(
            sphere=non_root_sphere, title="Secret", content="body", is_published=False
        )
    ...
    def test_announcement_scoped_to_current_sphere(self, client, sphere):
        Announcement.objects.create(sphere=sphere, title="Elsewhere", content="body")
    ...
    def test_unpublished_event_hidden_for_anonymous(self, client, non_root_sphere):
        EventFactory(sphere=non_root_sphere, publication_time=None)
    ...
    def test_unpublished_event_visible_for_manager_and_superuser(
        ...
        event = EventFactory(sphere=non_root_sphere, publication_time=None)
...
class TestEventsPageFeed:
    ...
    def test_merges_events_and_public_encounters_chronologically(
    ...
    def test_private_encounter_is_hidden_from_other_visitors(
        self, client, non_root_sphere
    ):
        EncounterFactory(
            sphere=non_root_sphere,
    ...
    def test_own_private_encounter_is_listed_for_its_organizer(
        self, authenticated_client, non_root_sphere, active_user
    ):
        encounter = EncounterFactory(
            sphere=non_root_sphere,
            creator=active_user,
    ...
    def test_past_encounters_join_past_events(self, client, non_root_sphere):
        now = datetime.now(UTC)
    ...
    def test_the_past_section_stops_at_one_cut_across_both_kinds(
        self, client, non_root_sphere
    ):
        now = datetime.now(UTC)
    ...
    def test_encounters_are_absent_while_the_sphere_runs_none(
        self, client, non_root_sphere
    ):
        EncounterFactory(
            sphere=non_root_sphere,
        ...
        non_root_sphere.encounters_policy = "none"
