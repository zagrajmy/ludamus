...
class TestEventPageView:
    ...
    def test_ok_compact_schedule_renders_all_row_variants(
        ...
        # `ongoing` below spans now-1h..now+1h; within an hour of local
        # midnight that window crosses into the next calendar date, and
        # build_schedule_days splits it into two tiles on two ScheduleDays —
        # one more than the single tile this test hardcodes. Freezing "now"
        # at local noon for both this fixture and the view's own request-time
        # clock keeps every relative offset here (up to +-3h) away from that
        # boundary; frozen after `event`/`space` exist, so their own dates
        # (set from the real clock by the fixtures) are untouched.
