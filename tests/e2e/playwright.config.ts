    ...
    ...(proxyServer
      ? {
          proxy: {
            ...
            // under test) and 127.0.0.1 must bypass it. `localhost` matches
            // that host alone, so the sphere subdomains the app serves
            // (another.localhost, …) need the suffix rule too.
