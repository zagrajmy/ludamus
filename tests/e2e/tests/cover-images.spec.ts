...
const coverDropzone = (page: Page) =>
  page.getByLabel("Cover image").locator("xpath=ancestor::label[@data-dropzone]");
...
test.describe("Event cover image upload", () => {
  ...
  test("manager removes a saved cover via the clear button", async ({ page }) => {
    ...
    // Clear it and save.
    await coverDropzone(page).getByRole("button", { name: "Remove image" }).click();
  ...
  test("rejects oversize file with error inside the dropzone", async ({ page }) => {
    await page.goto("/panel/event/lakeside-weekend/settings/");

    const oversize = Buffer.concat([PNG_BYTES, Buffer.alloc(8 * 1024 * 1024 + 1, 0)]);
    await page.getByLabel("Cover image").setInputFiles({
      name: "huge.png",
      mimeType: "image/png",
      buffer: oversize,
  ...
  test("rejects unsupported (GIF) format with error inside the dropzone", async ({ page }) => {
    await page.goto("/panel/event/lakeside-weekend/settings/");
