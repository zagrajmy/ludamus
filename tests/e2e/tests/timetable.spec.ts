...
test.describe("Timetable", () => {
  ...
  test("opens timetable page with grid and session list", async ({ page }) => {
    await page.goto("/panel/event/sunhaven-festival/timetable/");

    await expect(page.getByRole("heading", { name: "Schedule" })).toBeVisible();

    // Session list panel
    await expect(page.getByText("Sessions to assign")).toBeVisible();
  ...
  test("clicking a session card opens the detail view", async ({ page }) => {
    await page.goto("/panel/event/sunhaven-festival/timetable/");
    ...
    await expect(leftPane.getByText("RPG Introduction")).toBeVisible();
    await expect(leftPane.getByText("Not assigned")).toBeVisible();

    // Assign button should be present
    await expect(leftPane.getByRole("button", { name: "Assign" })).toBeVisible();
  });
  ...
  test("clicking Assign enters assignment mode with banner", async ({ page }) => {
    await page.goto("/panel/event/sunhaven-festival/timetable/");
    ...
    await expect(page.getByText("Click a position on the grid")).toBeVisible();

    // Grid columns should have assignment mode class
    await expect(page.locator(".timetable-column.assign-mode-active").first()).toBeVisible();
  });
  ...
  test("Escape key cancels assignment mode", async ({ page }) => {
    ...
    await expect(page.locator(".timetable-column.assign-mode-active")).toHaveCount(0);
  });
  ...
  test("assigns a session by clicking grid column", async ({ page }) => {
    ...
    const column = page.locator(".timetable-column.assign-mode-active").first();
    ...
    await expect(page.locator("#timetable-grid").getByText("RPG Introduction")).toBeVisible({
      timeout: 10000,
    });
  ...
  test("clicking scheduled session in grid shows Unassign button", async ({ page }) => {
    await page.goto("/panel/event/sunhaven-festival/timetable/");

    // Wait for grid to show the assigned session
    const gridSession = page.locator("#timetable-grid").getByText("RPG Introduction");
  ...
  test("unassigns a session", async ({ page }) => {
    ...
    // Click Unassign
    const leftPane = page.locator("#left-pane");
    await expect(leftPane.getByRole("button", { name: "Unassign" })).toBeVisible({ timeout: 5000 });
    ...
    await expect(page.locator("#timetable-grid").getByText("RPG Introduction")).not.toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("confirms and unconfirms a scheduled session", async ({ page }) => {
    ...
    const gridSession = page.locator("#timetable-grid").getByText("Storytelling Workshop");
    await expect(gridSession).toBeVisible({ timeout: 10000 });
    await gridSession.click();
  ...
  test("activity log page loads and shows tab navigation", async ({ page }) => {
  ...
  test("activity log offers Revert only on the latest change per session", async ({ page }) => {
    ...
    await expect(latestAssignRow.getByRole("button", { name: "Revert" })).toBeVisible();

    // Now unassign the same session — this supersedes the assign entry.
    await page.goto("/panel/event/sunhaven-festival/timetable/");
    await page.locator("#timetable-grid").getByText("Dungeon Crawl").click();
    await expect(leftPane.getByRole("button", { name: "Unassign" })).toBeVisible({ timeout: 5000 });
    await leftPane.getByRole("button", { name: "Unassign" }).click();
    await expect(
      page.getByRole("region", { name: "Sessions to assign" }).getByText("Dungeon Crawl"),
    ...
    await expect(latestRemovedRow.getByRole("button", { name: "Revert" })).toBeVisible();
    ...
    await expect(supersededAssignRows.getByRole("button", { name: "Revert" })).toHaveCount(0);
  });

  // --- Overview Page ---

  test("overview page loads with heatmap and track progress", async ({ page }) => {
    await page.goto("/panel/event/sunhaven-festival/timetable/overview/");
  ...
  test("overview page reports hours left to fill", async ({ page }) => {
    await page.goto("/panel/event/sunhaven-festival/timetable/overview/");

    // Hours-to-fill section with its capacity stats.
    await expect(page.getByRole("heading", { name: "Hours to fill" })).toBeVisible();
  ...
  test("tabs navigate between timetable sub-pages", async ({ page }) => {
    await page.goto("/panel/event/sunhaven-festival/timetable/");
