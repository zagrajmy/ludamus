...
test.describe("Proposal delete and restore", () => {
  ...
  test("soft-deletes a pending session from its detail page", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Recently deleted" })).toHaveCount(0);
    ...
    await expect(page.getByRole("link", { name: PROPOSAL_TITLE })).toHaveCount(0);

    await expect(page.getByRole("heading", { name: "Recently deleted" })).toBeVisible();
    const deletedRow = page.getByRole("row").filter({ hasText: PROPOSAL_TITLE });
