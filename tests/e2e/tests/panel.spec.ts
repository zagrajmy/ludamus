...
test.describe("Backoffice Panel", () => {
  ...
  test("creates a nested space inside a parent", async ({ page }) => {
    await page.goto("/panel/event/frostfire-con/venues/");
    // The duplicate-space test later in this serial block leaves an "Aurora
    // Convention Hall (Copy)" behind, so on a replay the substring match
    // resolves to two links; exact keeps this on the original venue.
    await page
