...
| `PersonalDataFieldValue` (per-event) | per-event values | R U        | R U (via `FacilitatorEditPageView`)  | OK — stays editable. Step 6 also exposes inline edit on the proposal-edit page.                      |
...
- **Shipped (read + write in one pass).** Each assigned facilitator gets
  ...
  sourced from the same `personal_data_fields` + `personal_data_field_values`
...
- **New service** `PersonalDataFieldValueService.update_personal_data(*,
  event_id, facilitator_id, entries)` in
  `mills/submissions/personal_data_fields.py`, wired onto
  `request.services.personal_data_field_values`. It owns the transaction and
  event-scoping (`facilitators.read(id).event_id == event_id` else
  `NotFoundError`), then calls `PersonalDataFieldValueRepository.save`. The
...
- Documentation deferred: the plan itself documents the edit surface;
  ...
  services are `session_content_edit` and `personal_data_field_values`).
...
`PersonalDataFieldValue` editing on the dedicated facilitator-edit page stays
as today.
...
2. **`PersonalDataFieldValue`** — personal-data answers, including the Step 6
...
- New **`FacilitatorChangeLog`** model (event + facilitator + user +
  ...
  required `session` FK.
- `PersonalDataFieldValueService` now owns the audit: `_diff_personal_data`
  ...
  `uow.facilitators.update` / `uow.personal_data_field_values.save` onto the
