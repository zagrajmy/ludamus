...
def interval_windows(
    *, start: datetime, end: datetime, tz: tzinfo, day_start_hour: int = 0
    ...
    first_date = local_date(start, tz=tz, day_start_hour=day_start_hour)
    last_date = local_date(end, tz=tz, day_start_hour=day_start_hour)
    windows: list[SlotWindow] = []
    for offset in range((last_date - first_date).days + 1):
        cursor_date = first_date + timedelta(days=offset)
        day_start = day_opening(cursor_date, tz=tz, day_start_hour=day_start_hour)
        day_end = day_opening(
            cursor_date + timedelta(days=1), tz=tz, day_start_hour=day_start_hour
        )
...
def programme_date(instant: datetime, tz: tzinfo) -> date:
    return local_date(instant, tz=tz, day_start_hour=PROGRAMME_DAY_STARTS_AT_HOUR)


def programme_day_start(day: date, tz: tzinfo) -> datetime:
    # The instant a programme day opens: its name and date are the day's.
    return day_opening(day, tz=tz, day_start_hour=PROGRAMME_DAY_STARTS_AT_HOUR)
